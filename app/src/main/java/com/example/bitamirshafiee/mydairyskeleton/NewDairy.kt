...
import com.example.bitamirshafiee.mydiaryskeleton.R
...
class NewDiary : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_new_dairy)//R.layout.activity_new_diary)
        ...
        current_date_dairy.text = currentDate.format(Date())
    ...
    private fun readDiary(id: Int){
        ...
        while(cursor.moveToNext()){
            ...
            current_date_dairy.text = currentDate
            title_dairy.setText(currentTitle)
            dairy_text.setText(currentDiary)
    ...
    private fun insertDiary(){
        ...
        val dateString = current_date_dairy.text.toString()
        val titleString = title_dairy.text.toString().trim(){it <= ' '}
        val diaryString = dairy_text.text.toString().trim(){it <= ' '}
        ...
        if (rowId.equals(-1)){
            Toast.makeText(this, "Sorry Can't Save", Toast.LENGTH_SHORT).show()
        }else{
    ...
    private fun updateDiary(id : Int){
        ...
        val values = ContentValues().apply {
            put(COLUMN_TITLE, title_dairy.text.toString())
            put(COLUMN_DIARY, dairy_text.text.toString())
        }
